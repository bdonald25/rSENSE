class ProjectsController < ApplicationController
  ...
  def save_fields
    ...
    (params[:hidden_num_count].to_i).times do |i|
      if addField('Number', params[('number_' + (i + 1).to_s).to_sym], params[('units_' + (i + 1).to_s).to_sym], [],  params['number_' + (i + 1).to_s + '_index']) == -1 and return
      end
    end
    ...
    (params[:hidden_text_count].to_i).times do |i|
      ...
      if addField('Text', params[('text_' + (i + 1).to_s).to_sym], '', restrictions, params['text_' + (i + 1).to_s + '_index']) == -1 and return
      end
    end
